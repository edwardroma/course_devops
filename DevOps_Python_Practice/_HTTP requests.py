...
# 1. Basic GET Request To fetch data from an API endpoint using a GET request:
import requests
response = requests.get('https://api.example.com/data')
data = response.json()  # Assuming the response is JSON
print(data)

# 2. GET Request with Query Parameters. To send a GET request with query parameters:
import requests
...
# 3. Handling HTTP Errors. To handle possible HTTP errors gracefully:
import requests
response = requests.get('https://api.example.com/data')
try:
